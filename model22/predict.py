...
y_pred_df = clf.decision_function(X)
y_pred_class = clf.predict(X)
#y_pred = clf.predict_proba(X)
y_pred_class_df = pd.DataFrame(y_pred_class,columns=['solved_status'])
submit = pd.concat([submit,y_pred_class_df], axis=1)
submit.to_csv("test_predict_class.csv", index=False)

np.savetxt("test_df.csv", y_pred_df, delimiter=',')
